...
You might have already used an `Integrated Development Environment (IDE) <https://en.wikipedia.org/wiki/Integrated_development_environment>`__ to write Python programs. In this course, we will do the same for programming in Java: We will use the free and open-source version of `IntelliJ IDEA <https://en.wikipedia.org/wiki/IntelliJ_IDEA>`__ (simply referred to as "IntelliJ" hereafter). Evidently, this isn't the only free and open-source IDE available for Java. Other popular choices include `NetBeans <https://en.wikipedia.org/wiki/NetBeans>`__ and `Eclipse <https://en.wikipedia.org/wiki/Eclipse_(software)>`__.

You won't need the proprietary edition of IntelliJ. Instead, download precompiled binaries from  `<https://github.com/JetBrains/intellij-community/releases>`_ (scroll down to find the latest release of ``idea``, such as ``idea/2026.1.1``, and open the list of assets). Uncompress the archive corresponding to your platform (e.g., ``idea-2026.1.1.tar.gz`` for GNU/Linux or ``idea-2026.1.1.win.zip`` for Microsoft Windows), you will be able to start IntelliJ by running the executable in the ``bin/`` folder.
